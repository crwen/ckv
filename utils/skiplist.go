...
func (list *SkipList) levelPrint() {
	maxLevel := list.maxLevel
	header := list.arena.getElement(list.headOffset)
	for i := maxLevel - 1; i >= 0; i-- {
		for curr := list.getNext(header, i); curr != nil; {
			vo, vs := decodeValue(curr.value)
			fmt.Printf("(%s, %s) -> ", curr.key(list.arena), list.arena.getVal(vo, vs).Value)
			curr = list.getNext(curr, int(i))
		}
		fmt.Println()
	}
}
